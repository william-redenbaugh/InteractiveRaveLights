...
{
    ipc_subscrube_module_t *mod = (ipc_subscrube_module_t*)pvPortMalloc(sizeof(ipc_subscrube_module_t));
...
bool _ipc_run_all_sub_cb(ipc_message_header_t header, uint8_t *data, ipc_subscrube_module_t *mod)
{
    ...
    xSemaphoreTake(list.ipc_message_node_muttx, portMAX_DELAY);
    ...
    xSemaphoreGive(list.ipc_message_node_muttx);
...
bool _ipc_attach_cb(ipc_subscrube_module_t *mod, int message_id, ipc_sub_cb specified_cb)
{
    ...
    if (node == NULL)
    {
        list.head = (ipc_subscribe_cb_node_t *)pvPortMalloc(sizeof(ipc_subscribe_cb_node_t));
        list.head->next = NULL;
    ...
    else
    {
        ...
        node->next = (ipc_subscribe_cb_node_t *)pvPortMalloc(sizeof(ipc_subscribe_cb_node_t));
        node = node->next;
        node->next = NULL;
